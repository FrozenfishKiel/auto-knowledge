...
async def scheduler_worker_loop(app) -> None:
    ...
    while True:
        try:
            ...
            if await Config.get('auto_knowledge.enable', True):
                try:
                    from open_webui.utils.auto_knowledge.scheduler import claim_and_execute_due_jobs

                    claimed = await claim_and_execute_due_jobs(app)
                    if claimed:
                        log.info(f'Claimed {claimed} due auto knowledge job(s)')
                except Exception:
                    log.exception('Scheduler: auto knowledge error')
